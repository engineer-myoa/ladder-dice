...
def __grantScore__(size, iterateCount, masterSeed):
    ...
    for seedVal in generatedNewSeedList:
        ...
        accumulationScoreResult.append(
            accumulationScoreResult[-1] + np.random.randint(0, adaptiveMaxScore, size, np.uint64))
...
def __dictToSortedList__(dictionary: dict, limit=None):
    print("lim1 : {0}".format(limit))
    limit = __optionalIndexBound__(limit, dictionary)

    print("lim2 : {0}".format(limit))
...
def prettyGraph(dictionary, limit=None):
    ...
    fontName = fontmgr.FontProperties(fname=FONT_PATH, size=16).get_name()
    ...
    plt.savefig(PREFIX_GRAPH_PATH + fileName)
    return fileName
...
def prettyPrint(dictionary, requestedTime, limit=None):
    ...
    sumScore = np.sum(np.fromiter([eachUserScore[-1] for _, eachUserScore in totalScoreList],
                                  np.uint64))  # get last elements and summation all

    for k, v in totalScoreList:
        v = v[-1]  # get last item
        ratio = v / sumScore
        percentage = ratio * 100
        resultString += "{0:<15} :: {1:<{2}} :: {3} pts ({4:2.2f}%)".format(k, __drawAsciiArt__(ratio),
                                                                            int(ASCII_MAX_LENGTH * 0.5), v,
                                                                            percentage) + "\n"
...
def generateRandomDice(users, iterateCount=None, requestedTime=None):
...
if __name__ == "__main__":
    ...
    userDict = generateRandomDice(['한글a', '영문b', 'c', 'd', 'e', 'f'], None, requestedTime)
